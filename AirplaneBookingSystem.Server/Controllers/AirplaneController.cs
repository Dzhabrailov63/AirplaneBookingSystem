...
public class AirplaneController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<int> PostAirplane([FromBody] AirplanePostDto airplane)
    {
        var ctx = await _contextFactory.CreateDbContextAsync();
        _logger.LogInformation("Create new airplane");
        var addedAirplane = await ctx.Airplanes.AddAsync(_mapper.Map<Airplane>(airplane));
        await ctx.SaveChangesAsync();

        return addedAirplane.Entity.Id;
    }
    /// <summary>
    /// Delete method 
    /// </summary>
