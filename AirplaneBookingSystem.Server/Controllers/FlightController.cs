...
public class FlightController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<int> PostFlight([FromBody] FlightPostDto flight)
    {
        var ctx = await _contextFactory.CreateDbContextAsync();
        _logger.LogInformation("Create new flight");
        var addedFlight = await ctx.Flights.AddAsync(_mapper.Map<Flight>(flight));
        await ctx.SaveChangesAsync();

        return addedFlight.Entity.Id;
    }
    /// <summary>
    /// Delete method 
    /// </summary>
