...
public class TicketController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<int> PostTicket([FromBody] TicketPostDto ticket)
    {
        var ctx = await _contextFactory.CreateDbContextAsync();
        _logger.LogInformation("Create new ticket");
        var addedTicket = await ctx.Tickets.AddAsync(_mapper.Map<Ticket>(ticket));
        await ctx.SaveChangesAsync();

        return addedTicket.Entity.Id;
    }
    /// <summary>
    /// Delete method 
    /// </summary>
