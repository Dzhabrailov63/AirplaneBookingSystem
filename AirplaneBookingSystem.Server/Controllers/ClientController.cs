...
public class ClientController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<int> PostClient([FromBody] ClientPostDto client)
    {
        var ctx = await _contextFactory.CreateDbContextAsync();
        _logger.LogInformation("Create new client");
        var addedClient = await ctx.Clients.AddAsync(_mapper.Map<Client>(client));
        await ctx.SaveChangesAsync();

        return addedClient.Entity.Id;
    }
    /// <summary>
    /// Delete method 
    /// </summary>
